...
- [ ] **🔴 CRITICAL — Renovate is OFFLINE (external fix).** The Mend Cloud Renovate app stopped running ~2026-04-04 (Dependency Dashboard #85 frozen at that date, zero `renovate/*` branches, no recent bot PRs). Every dependency is unmanaged and all `renovate.json` automerge/grouping/buffer config is **inert** until the app is reactivated. This is why the pnpm 11 + vite/undici/brace-expansion CVE bumps had to be applied by hand this session. Reinstall at <https://github.com/apps/renovate> (or check repo status at developer.mend.io). Once revived, it auto-catches the drift below in one cycle. Drift accumulated while offline: ~14 npm deps (react 19.2.4→19.2.7, vitest/coverage 4.1.5→4.1.9, eslint 10.3→10.5, typescript-eslint, prettier, terser, etc.; @types/node 25→26 major), 2 mise tools (act 0.2.88→0.2.89, **trivy 0.70.0→0.71.2** — the CVE scanner), 2 base images (**nginx 1.30.0→1.30.3**, node 24.15.0→24.17.0), 8 GitHub Action digests.
- [ ] **Dependabot alerts are OFF** (`gh api repos/AndriyKalashnykov/viteapp/vulnerability-alerts` → 404), so `renovate.json`'s `vulnerabilityAlerts` security fast-track block is inert. CVE coverage currently relies on the CI scanners (`trivy-fs`, Trivy image scan, `pnpm audit`/`vulncheck`). Enable with `gh api -X PUT repos/AndriyKalashnykov/viteapp/vulnerability-alerts` to make the block live (read-only Security-tab signal; no extra PRs on its own).
- [ ] **`automergeType: branch` vs the `main` ruleset** — `main` now has an active ruleset with 1 required status check (empty bypass). Branch-mode automerge pushes directly to `main`, which the Renovate bot (not a bypass actor) cannot do against a required check → routine bumps would silently fail to automerge once Renovate is revived. Either keep `branch` and drop the required check, or switch to `automergeType: pr` (+ `platformAutomerge: false` to dodge the registration race). Deferred for an explicit decision — branch mode was a deliberate choice to reduce email noise.
