...
- Triggers: push to `main`, tags `v*`, pull requests, `workflow_call` (reusable), and `workflow_dispatch` (re-fired by `diagram-regen.yml` after a commit-back — see below). No trigger-level `paths-ignore` — uses a `changes` detector job (`dorny/paths-filter`) so doc-only changes skip heavy work without deadlocking Repository Rulesets that require `ci-pass`. On `workflow_dispatch` the `changes` job forces `code=true` (a dispatch has no diff base for paths-filter and only ever fires after a render-affecting regen).
...
- Triggers on `pull_request` (`opened`/`synchronize`/`reopened`) from `renovate[bot]` that touches `docs/diagrams/**/*.puml` or `Makefile` (which carries `PLANTUML_VERSION`). Solves the structural problem that **Renovate (Mend Cloud) cannot run `make diagrams`**: a render-affecting bump (the C4-PlantUML `!include` version, the `plantuml/plantuml` image) drifts the committed PNG from source, so `diagrams-check` fails and — under `automerge: true` — the PR would sit perpetually RED and never merge until a human regenerated by hand.
- The job runs `make diagrams`, and if the committed PNG drifted, commits it back to the PR head and re-fires `ci.yml` via `gh workflow run ci.yml --ref <branch>`. A GITHUB_TOKEN push does **not** trigger `pull_request` (GitHub anti-recursion), so the explicit `workflow_dispatch` is what produces the `ci-pass` required check on the new head SHA → the bump PR self-heals green → Renovate automerges, hands-off.
- Permissions: `contents: write` (push) + `actions: write` (dispatch); no `pull-requests: write`, no `packages: write`. Scoped to `renovate[bot]` (fork PRs get a read-only token anyway); a recursion guard skips when the branch tip is already `github-actions[bot]`. No new secret required. Skipped under act (not exercised by `make ci-run`).
...
- **Architecture diagrams (PlantUML C4):** the README hero is a C4 Context diagram authored in `docs/diagrams/c4-context.puml` (C4-PlantUML `!include` version-pinned + Renovate-tracked via a `custom.regex` manager, modern-flat theme) and rendered to a committed PNG under `docs/diagrams/out/` via the pinned `plantuml/plantuml` Docker image (`PLANTUML_VERSION` in the Makefile, Renovate-tracked). `make diagrams` renders; `make diagrams-check` (a `git diff --exit-code` drift gate in `static-check`) fails if the committed PNG diverges from current source. A version-stamped sentinel (`docs/diagrams/out/.plantuml-*.stamp`, gitignored) forces a full re-render on a `PLANTUML_VERSION` bump so a renderer bump can't ship stale PNGs. Because Renovate can't run `make diagrams`, a render-affecting bump (the `!include` version or `PLANTUML_VERSION`) would otherwise fail `diagrams-check` and never automerge — `diagram-regen.yml` (see CI/CD) renders + commits the PNG back to the bump PR and re-fires the gate so it self-heals. Rendering is skipped under act (DinD bind-mount), same as mermaid-lint
