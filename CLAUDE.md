...
Last reviewed: 2026-07-29 (override-key semantics corrected; lockFileMaintenance daily)
...
- [ ] **CORRECTED 2026-07-29 — a red `pnpm audit` on these transitive CVEs is fixed by a LOCKFILE REFRESH, NOT by bumping an override floor.** The two prior entries here (and the comment in `pnpm-workspace.yaml`) told the opposite story and it caused three incidents, the last of which left `main` red for 4 days with 7 PRs blocked behind it (GHSA-mh99-v99m-4gvg, brace-expansion, patched ≥5.0.8). **Measured against the pinned pnpm 11.15.1** (`dist/pnpm.mjs` — `overrideDeps` iterates `Object.entries(deps)`, a manifest map of name→**declared range**, and filters with `isIntersectingRange(targetPkg.bareSpecifier, bareSpecifier)`): pnpm matches an override KEY against the **dependent's declared range** via `semver.intersects`, **never** against the resolved version. Consequences: (1) the overrides were **live** the whole time, and their caret targets (`^5.0.7`, `^7.28.0`) **already admitted** the patched 5.0.8 / 7.29.0 — so they were never what was red; the lockfile was the only thing pinning an old version. (2) The previous two "floor bump" fixes were unnecessary and worked by accident: pnpm records `overrides:` **inside** `pnpm-lock.yaml`, so editing one invalidates the lockfile and forces a re-resolve — a cache-bust wearing a security pin's clothes. **Remedy for the next occurrence: `pnpm update <pkg>` (or let `lockFileMaintenance` do it), not an override edit.**
- [ ] **Override keys are now ceiling-free** (`"undici@^7.0.0": "^7.29.0"`, `"brace-expansion@^5.0.0": "^5.0.8"`). A key with an upper bound can stop **intersecting** the dependent's declared range and then silently stops applying — e.g. `>=5.0.0 <5.0.7` against a future minimatch declaring `^5.1.0` intersects nothing. The caret targets auto-heal within the major, so no future 5.x/7.x advisory needs an edit here. Re-evaluate only on a dependent's MAJOR move: jsdom → undici 8.x (also drop the `allowedVersions: <8` cap in `renovate.json`, which exists because jsdom 29.1.1 hard-requires `undici/lib/handler/wrap-handler.js` — removed in v8; verified by PR #303 breaking the vitest/jsdom runtime), or minimatch → brace-expansion 6.x.
- [ ] **KNOWN GAP — no periodic security signal.** `ci.yml` has no `schedule:` trigger, so a newly-disclosed advisory against an **unchanged** `main` produces **zero** signal until some PR happens to run CI. That is exactly how the 2026-07-29 incident stayed invisible for 4 days. `lockFileMaintenance` is now daily (was the weekly default), which shrinks the *refresh* window to ~1 day, but it is **not detection**. Also note `trivy-fs` is **not** a second net for this class — it suppresses dev-dependencies, so on `pnpm-lock.yaml` its vuln leg scans **3 of 321 packages** (measured; `--include-dev-deps` gives the 321). For a static SPA where everything but react/react-dom is dev-side, that is ~1% coverage, so routine-CI JS security rests entirely on `pnpm audit` — the image scan being tag-only (above) is the other half of that. Revisit (a nightly `static-check`) if this recurs; a naive `schedule:` trigger needs `code=true` forced for schedule events, or `dorny/paths-filter` computes an empty base, `static-check` **skips**, and `ci-pass` goes green having looked at nothing.
- [ ] **`vulnerabilityAlerts` is INERT for these deps** (graded `source-read`, not run). Sharper than "needs a manifest entry": Renovate *does* extract each `pnpm-workspace.yaml` override as a dep with `packageName: brace-expansion`, so an entry exists — but `workers/repository/process/vulnerabilities.js` computes `depVersion = lockedVersion ?? currentVersion ?? currentValue` and bails on `if (!versioningApi.isVersion(depVersion))`. For an override dep, `currentValue` is a **range** (`^5.0.8`) and `lockedVersion` is not populated, so the vulnerability lookup is skipped outright. The GitHub-alert path only emits a `packageRule` (`matchCurrentVersion`), which can constrain an update but cannot manufacture one. Consistent with all three incidents being resolved manually. Settle end-to-end with `GITHUB_TOKEN=… make renovate 2>&1 | grep -i 'Skipping vulnerability lookup'`.
