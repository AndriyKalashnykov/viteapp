...
- [ ] **KNOWN GAP — no periodic security signal.** `ci.yml` has no `schedule:` trigger, so a newly-disclosed advisory against an **unchanged** `main` produces **zero** signal until some PR happens to run CI. That is exactly how the 2026-07-29 incident stayed invisible for 4 days. `lockFileMaintenance` is now daily (was the weekly default), which shrinks the *refresh* window to ~1 day, but it is **not detection**. Also note `trivy-fs` is **not** a second net for this class — it suppresses dev-dependencies, so it never sees an eslint-transitive CVE. Revisit (a nightly `static-check`) if this recurs; a naive `schedule:` trigger needs `code=true` forced for schedule events, or `dorny/paths-filter` computes an empty base, `static-check` **skips**, and `ci-pass` goes green having looked at nothing.
- [ ] **UNVERIFIED — `vulnerabilityAlerts` may be inert for purely-transitive deps.** Renovate's security fast-track needs a manifest entry to bump; `brace-expansion` and `undici` have none (neither is in `package.json`). Consistent with all three incidents being resolved manually. Settle with a `make renovate` dry-run and grep the log; if confirmed, say so in `renovate.json` so nobody counts on it.
