...
- [ ] **`make check-dockerfile-stage` still does NOT cover two builder shapes.** The Makefile half is done (it now asserts both COVERAGE — a `buildx build --cache-from` lacking `--no-cache-filter` fails — and VALUE — the stage name matches), so the gate reports two denominators. Still uncounted: (a) a `docker buildx build --cache-from` inside a **workflow `run:` step**, and (b) any builder under **`scripts/`**. Neither exists today; both would pass silently. The gate's PASS line carries a scope comment so its denominators aren't read as complete coverage.
